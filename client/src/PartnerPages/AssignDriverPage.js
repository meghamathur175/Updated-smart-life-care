...
import '../styles/AssignDriverPage.css';

const AssignDriver = () => {
  const [drivers, setDrivers] = useState([]);
  const [loading, setLoading] = useState(false);
  const [alreadyAssignedByThis, setAlreadyAssignedByThis] = useState(0); // updated
  ...
  const fetchAssignedCount = async () => { // updated
    try {
      ...
      if (latest) {
        const count = latest?.assignedAmbulances?.filter(
          (a) => a.partnerId === partnerId
        ).length;
        setAlreadyAssignedByThis(count || 0);
      }
    } catch (err) {
  ...
  const handleAssignDriver = async (driverId) => {
    ...
    if (!selectedDriver) {
      alert("Driver not found in available list.");
      return;
    }

    const totalRequested = requestData?.numberOfAmbulancesRequired || 1; // updated
    if (alreadyAssignedByThis >= totalRequested) { // updated
      alert("❌ You have already assigned the required number of ambulances.");
      return;
    }
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/partner-drivers/assign-driver', {
        ...
        ambulance: {
          driverId,
          plateNumber: selectedDriver?.ambulancePlateNumber || "",
