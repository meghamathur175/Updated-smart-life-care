  ...
  useEffect(() => {
    const fetchPartnerDrivers = async () => {
      try {
        ...
        const res = await axios.get(`http://localhost:3001/api/drivers/by-partner?partnerId=${partnerId}`);
        console.log("Response partner driver: ", res);
        ...
        <tbody>
          {partnerDrivers.map((driver, index) => (
            <tr
              key={driver._id}
              onClick={() =>
                navigate("/partner-dashboard/partner-driver-details", {
                  state: { driver },
                })
              }
