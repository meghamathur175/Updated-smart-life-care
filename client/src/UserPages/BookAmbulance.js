  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      console.log("Sending confirm request with:", {
        ...
        urgency,
        ambulanceType,
        ambulanceCost: ambulanceCost + hospital.cost,
      });
      ...
      const response = await fetch("http://localhost:3001/api/partners/confirm-request", {
        ...
        body: JSON.stringify({
          ...
          urgency,
          ambulanceType,
          ambulanceCost: ambulanceCost + Number(hospital.cost.replace("₹", "").trim()),
        }),
      });

      console.log("Response from confirm-request: ", response)
      console.log("Ambulance total cost: ", ambulanceCost + Number(hospital.cost.replace("₹", "").trim()))


      const data = await response.json();
      console.log("DATA from confirm-request: ", data)
      ...
      setTimeout(() => {
        ...
        navigate("/looking-for-driver", {
          state: {
            ...
            ambulanceTypeCost: ambulanceCost,
