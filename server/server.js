...
io.on("connection", (socket) => {
  ...
  socket.on("accept_request", async ({ requestId, driverId }) => {
    try {
      // Find the partner that has this pending request
      const partner = await Partner.findOne({ "pendingRequests.bookingId": requestId });
      if (!partner) return;

      // Find the request in the partner's pendingRequests array
      ...
      if (request && !request.assignedDriverId) {
        ...
        // Optionally, update driver status to unavailable
        await Driver.findByIdAndUpdate(driverId, { isAvailable: false });

        // Notify all drivers to close the pop-up for this request
