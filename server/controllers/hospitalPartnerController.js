...
const HospitalPartner = require("../models/HospitalPartnerModel");
const Partner = require("../models/PartnerModel");

// Abstract Email Validation Function
...
const loginHospitalPartner = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: hospitalPartner._id }, process.env.KEY, { expiresIn: "1d" });
...
const getLoggedInHospitalPartner = async (req, res) => {
  try {
    const partner = await HospitalPartner.findById(req.partnerId).select("-password");
    if (!partner) {
