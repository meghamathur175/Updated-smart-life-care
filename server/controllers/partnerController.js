...
exports.getPartnerByHospitalPlaceId = async (req, res) => {
  ...
  try {
    ...
    console.log("Place ID backend:", trimmedPlaceId);
    const partner = await Partner.findOne({ hospitalPlaceId: trimmedPlaceId });

    console.log("Partner backend: ", partner);
    if (!partner) {
      return res.status(404).json({ message: "Partner not found" });
    }
...
exports.confirmRequestToPartner = async (req, res) => {
  try {
    const { userId, userName, partnerId, pickup, drop } = req.body;

    // Step 1: Validate input
    if (!userId || !partnerId || !pickup || !drop || !userName) {
