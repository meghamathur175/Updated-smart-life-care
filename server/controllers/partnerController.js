...
exports.confirmRequestToPartner = async (req, res) => {
  try {
    ...
    const { partnerId, userId, userName, pickup, drop, urgency, ambulanceType, ambulanceCost, pickupDistanceKm, socketId } = req.body;
    ...
    // const alreadyExists = partner.pendingRequests?.some(req => req.bookingId === bookingId);
    // if (alreadyExists) {
    //   return res.status(400).json({ message: "Request with same bookingId already exists." });
    // }
...
exports.rejectRequest = async (req, res) => {
  try {
    ...
    mainRequest.reassignedPartners = Array.from(reassignedSet).map(id => new mongoose.Types.ObjectId(id));
    ...
    mainRequest.rejectionType = rejectionType || "manual";
    mainRequest.reassignedPartners = [...(mainRequest.reassignedPartners || []), partnerId];
...
exports.confirmHospitalSelection = async (req, res) => {
  try {
    const { selectedPartnerId, originalPartnerId, bookingId, previousReassignedPartners = [], pickup, drop, ambulanceType, urgency, userName } = req.body;
